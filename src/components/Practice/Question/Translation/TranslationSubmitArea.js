...
const TranslationSubmitArea = ({onSubmit, result, sentence, correct, answer, setSentenceCorrectness}) => {
    ...
    function AskGPT() {
        return (<button className="button gpt-btn" onClick={() => {
            setIsLoading(true);
        }}>
            {isLoading ? <div style={{display: "grid", placeItems: "center"}}><CircularProgress size="20px"
                                                                                                color="inherit"/>
            </div> : <svg width="25" height="25" viewBox="0 0 41 41" fill="none" xmlns="http://www.w3.org/2000/svg"
    ...
    return (<div className={`submit-wrapper ${correct.correct ? 'correct' : 'incorrect'}`}>
        <div className="feedback">
            ...
            <div>
                <div className="feedback-title">{correct.correct ? "Correct!" : "Incorrect"}&nbsp;
                    <div className="true-center"><ListeningButton
                        sentence={sentence} small={true}/></div>
                </div>
                <div>
        ...
        <div className="sentence-eval-buttons-wrapper">
            ...
            {!correct.correct && !correct.error && <>
                <button className="button change-result" onClick={() => {
                    handleCorrectClick();
                }}>The translation was actually correct!
                </button>
            </>}
            {correct.correct && !correct.exact && <>
                <button className="button change-result" onClick={() => {
                    handleIncorrectClick();
                }}>The translation was actually incorrect!
                </button>
            </>}
        </div>
    </div>);
